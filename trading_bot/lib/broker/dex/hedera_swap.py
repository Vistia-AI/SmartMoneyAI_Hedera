...
from lib.trading import Order, Trade, TradingBot, Strategy, BaseBroker, OrderPlan
import ulid
...
class SwapBroker(BaseBroker):
    def __init__(self, rpcs, ecosystem_token='WHBAR', contract_info:dict=None, abi_url:str='', router_address=None, factory_address=None):
        ...
        self.router_contract = self.gateway.eth.contract(
            address=Web3.to_checksum_address(self.contract_info.get('router')[0]),
            abi=self.contract_info.get('router')[1]
        )
        self.factory_contract = self.gateway.eth.contract(
            address=Web3.to_checksum_address(self.contract_info.get('factory')[0]),
            abi=self.contract_info.get('factory')[1]
        )
    ...
    def get_decimal(self, symbol: str):
        try:
            token = self.gateway.eth.contract(
                address=self.contract_info[symbol][0], 
                abi=self.contract_info[symbol][1]
            )
    ...
    def check_balance(self, bot: 'TradingBot', re_check=True) -> Tuple[float, float]:
        ...
        for t in [bot.currency] + bot.tokens:
            t_add, t_abi = self.contract_info[t]
            ...
            bot._token_balance[t]['qty']=token_contract.functions.balanceOf(bot.wallet['address']).call()
    ...
    def estimate(self, t_path, amount_in_wei:int, function ='getAmountsIn'):
        ...
        nt_add = self.contract_info[self.ecosystem_token][0]
        valid_path = [self.contract_info.get(t_path[0])[0]]

        for t in t_path[1:]:
            token2 = self.contract_info.get(t)[0]
    ...
    def get_allowance(self, wallet:dict, symbol: str):
        # print("get_allowance: ", wallet, symbol)
        if self.gateway is None:
            raise Exception('No self.gateway found')
        t_add, t_abi = self.contract_info[symbol]
        try:
    ...
    def approve_token(self, wallet:dict, symbol: str, amount: int = 10e6):
        amount = int(amount)
        if self.gateway is None:
            raise Exception('No self.gateway found')
        ...
        approve_txn = token_contract.functions.approve(
            ...
            "from": wallet['address'],
            "nonce": self.gateway.eth.get_transaction_count(wallet['address']),
            "gasPrice": self.gateway.eth.gas_price,
        })
        signed_approve_txn = self.gateway.eth.account.sign_transaction(approve_txn, private_key=wallet['private'])
    ...
    def swap_exact_out(self, wallet:dict, path:list=['WHBAR','USDC'], amount_out:int=1000000000000000000, amount_in_max:int=None):
        ...
        allowance = self.get_allowance(wallet, sell_token)
        # print("allowance - amount_in", Web3.from_wei(allowance,'ether'), Web3.from_wei(amount_in,'ether'))
        if allowance < amount_in_max:
            self.approve_token(wallet, sell_token, amount=int(amount_in_max*1.5))
        ...
        for i in range(10):
            try:
                # Signing transaction
                signed_txn = self.gateway.eth.account.sign_transaction(build_txn, private_key=wallet['private'])
    ...
    def swap_exact_in(self, wallet:dict, path:list=['WHBAR','USDC'], amount_in:int=1000000000000000000, amount_out_min:int=0):
        # remember estimate is not combined with gas fee, so it not accurate for price calculation 
        if amount_in < 0:
        ...
        if allowance < amount_in:
            self.approve_token(wallet, sell_token, amount=int(amount_in*1.5))
        # Get current gas price
        gas_price = self.gateway.eth.gas_price  # Returns gas price in wei
        # Init transaction parameters
        tx_params = {
    ...
    def update_order(self, order:Order, wait_update:bool=False):
        ...
        if receipt:
            ...
            if len(data) == 32:
                amount_in = int.from_bytes(data, "big")
            else:
                raise Exception("Invalid amount_in data")

            data = Web3.to_bytes(hexstr=HexStr(receipt['logs'][-1]['data'].hex()))
            if len(data) == 128:
                ...
                raise Exception("Invalid amount_out data")
...
if __name__ == "__main__":
    import yaml
    with open('configs/bsc_chain.yaml', 'r') as file:
        chain_info = yaml.safe_load(file)

    print("test Saucerswap Broker")
    chain = chain_info.get('testnet', {})
    explorer = chain.get('block_explorer', {})
    apikey = explorer.get('api-key', '')
    abi_url = explorer.get('api', '') + explorer.get('query', {}).get('abi', '')
    abi_url = abi_url.format(apikey=apikey, address='{address}')
    ...
    broker = SwapBroker(
        ...
        abi_url=abi_url,
        # router_address=chain.get('contracts').get('router')[0],
    ...
    bot.wallet = chain.get('wallet', {})


    # print(broker.get_pair_info(['WHBAR', 'USDC']))
    print(broker.check_balance(
        bot=bot,
    ))

    buy_path, amounts = broker.estimate(['USDC', 'WHBAR'], broker.to_wei('USDC', 1))
    print(buy_path, amounts)
    print(broker.estimate(['USDC', 'WHBAR'], int(amounts[-1]), function='getAmountsIn'))
    
    # print("swap_exact_out: \n", broker.swap_exact_out(
    #     wallet=bot.wallet,
    #     path=['WHBAR', 'USDC'],
    #     amount_out=broker.to_wei('USDC', 0.01),
    #     amount_in_max=None
    # ))
    ...
    #     wallet=bot.wallet,
    #     path=['WHBAR', 'USDC'],
    #     amount_in=broker.to_wei('WHBAR', 0.01),
    #     amount_out_min=None
    # ))
