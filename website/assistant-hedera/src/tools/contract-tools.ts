...
// Simple Hedera-compatible utility functions
...
// Hedera zero address (placeholder)
...
export const depositToContractTool = tool({
  ...
  execute: async function (params) {
    ...
    try {
      ...
      const decimals = fromTokenAddress === ZERO_ADDRESS ? 18 : await getTokenDecimals(chainId, fromTokenAddress);
      ...
      if (fromTokenAddress !== ZERO_ADDRESS) {
        ...
        if (balances.walletBalance && BigInt(balances.walletBalance.allowance) < BigInt(amountIn)) {
          const symbol = await getTokenSymbol(chainId, fromTokenAddress as `0x${string}`);
      ...
      const symbol =
        fromTokenAddress === ZERO_ADDRESS ? 'S' : await getTokenSymbol(chainId, fromTokenAddress as `0x${string}`);
      ...
      return {
        ...
        deposit: {
          ...
          symbol,
          decimals,
          isNative: fromTokenAddress === ZERO_ADDRESS,
        },
      };
    } catch (error) {
    ...
    try {
      ...
      if (params.amount.toLowerCase() === 'max' || params.amount.toLowerCase() === 'all') {
        ...
        if (!contractBalance || contractBalance.amount === 0n) {
          ...
          const symbol = tokenAddress === wAddress ? 'S' : await getTokenSymbol(chainId, tokenAddress as `0x${string}`);
      ...
      // Execute withdrawal
      const withdrawResult = await executeWithdraw(session.address as `0x${string}`, tokenAddress as `0x${string}`, amountIn);
